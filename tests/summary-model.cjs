...
(async () => {
 ...
 let r=await ai.openAiTextStream('body',[{role:'user',content:'hello'}],undefined,100,true,'s1'); for await(const _ of r.text){}
 assert.equal(calls.at(-1).body.model,'claude-opus-4-6');
 await ai.generateAiText('summary',[{role:'user',content:'hello'}],100,undefined,{kind:'요약',sessionId:'s1'});
 assert.equal(calls.at(-1).body.model,'claude-sonnet-4-6');
 await configure('anthropic',{summaryModel:'claude-haiku-4-5-20251001',recentTurns:4});
 assert.equal(settings.getAiSettings().recentTurns,4);
 await ai.generateAiText('summary',[{role:'user',content:'hello'}],100,undefined,{kind:'요약'});
 assert.equal(calls.at(-1).body.model,'claude-haiku-4-5-20251001');
 await configure('anthropic',{summaryModel:'same'});
 await ai.generateAiText('summary',[{role:'user',content:'hello'}],100,undefined,{kind:'요약'});
 assert.equal(calls.at(-1).body.model,'claude-opus-4-6');
 assert.equal((await configure('anthropic',{summaryModel:'bad'})).status,400);
 console.log('PASS summary routing Sonnet/Haiku/same, unchanged Opus body, persisted settings and validation');
